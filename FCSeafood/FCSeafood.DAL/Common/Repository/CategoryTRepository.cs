...
public class CategoryTRepository : Base.BaseRepository<CategoryTDbo> {
    ...
    protected override IQueryable<CategoryTDbo> NoTracking() => this.Entities.Include(x => x.SubcategoryTDbos).AsNoTracking();
    ...
    public static (bool success, CategoryTModel model) ToModel(CategoryTDbo dbo) {
        ...
        if (subcategoryTListModel.success) model.SubcategoryTModelList = subcategoryTListModel.models.ToList();
