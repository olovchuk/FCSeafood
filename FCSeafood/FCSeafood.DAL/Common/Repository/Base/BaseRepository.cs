...
public abstract class BaseRepository<TEntity> where TEntity : class {
    private readonly ILogger _logger = LoggerFactory.Create(b => { b.AddConsole(); }).CreateLogger(typeof(TEntity));
    ...
    public virtual async Task<IReadOnlyCollection<TEntity>> GetAllAsync(bool skipNoneValues = true) {
        try {
            if (skipNoneValues)
                return await this.NoTracking().Skip(1).ToListAsync().ConfigureAwait(false);
            else
    ...
    public virtual async Task<IReadOnlyCollection<TEntity>> FindByConditionListAsync(
