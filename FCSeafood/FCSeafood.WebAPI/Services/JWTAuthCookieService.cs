...
public class JWTAuthCookieService {
    ...
    public async Task<SignInResponse> SignInAsync(SignInParams signInParams) {
        ...
        _cookieHelper.SetCookie(_configuration.GetValue<string>("TokenKeys:Access"), result.JWTAuthModel.AccessToken, TimeSpan.FromSeconds(_jwtAuthSettings.TokenLifeTime), true);
        _cookieHelper.SetCookie(_configuration.GetValue<string>("TokenKeys:Refresh"), result.JWTAuthModel.RefreshToken, TimeSpan.FromSeconds(_jwtAuthSettings.TokenLifeTime), true);
        return result;
    }
